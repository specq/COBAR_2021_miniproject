...
def create_data_set(beh_df, neural_df, trial_start, trial_stop, nb_events, filename):
    ...
    for k in range(trial_start, trial_stop):
        ...
        for i in range(nb_samples - nb_events):
            x[k-trial_start, i] = delta_F_over_F[i:i+nb_events]
            y[k-trial_start, i] = beh_joints_red[i:i+nb_events]
...
create_data_set(beh_df, neural_df, 0, nb_trials_train, nb_events, "train_set.pkl")
create_data_set(beh_df, neural_df, nb_trials_train, 
                nb_trials_train+nb_trials_val, nb_events, "val_set.pkl")
create_data_set(beh_df, neural_df, nb_trials_train+nb_trials_val, 
                nb_trials_train+nb_trials_val+nb_trials_test, nb_events, "test_set.pkl")
